...
/// 지난 날짜를 흐리게 덮는 최상단 오버레이. 그리드·이벤트 바 위에 그려 날짜·배경·이벤트를 한꺼번에 흐리게 만든다.
struct PastDatesVeil: View {
    let year: Int
    let theme: Theme
    var startMonth: Int = 1
    var monthsShown: Int = 12

    /// 과거 날짜 셀 위에 덮는 베일 불투명도 — 배경색으로 덮어 흐리게 만든다.
    private static let veilOpacity: Double = 0.55

    var body: some View {
        Canvas { context, size in
            guard let today = Self.todayComponents() else { return }
            let layout = CalendarLayout(size: size, monthsShown: monthsShown, startMonth: startMonth, year: year)
            let veil = Color(hex: theme.backgroundColor).opacity(Self.veilOpacity)
            let endMonth = startMonth + monthsShown - 1
            for month in startMonth...endMonth {
                let days = layout.daysInMonth(month)
                for day in 1...days where isPast(month: month, day: day, today: today) {
                    context.fill(Path(layout.fullCellRect(month: month, day: day)), with: .color(veil))
                }
            }
        }
    }
    ...
    private static func todayComponents() -> (year: Int, month: Int, day: Int)? {
